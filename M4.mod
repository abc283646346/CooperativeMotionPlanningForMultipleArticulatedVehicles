...
minimize objective_:
sum{i in {1..Nv}, j in {1..(Nfe-1)}}((v[i,1,j]-v[i,1,j-1])^2 + (phy[i,j]-phy[i,j-1])^2);
...
s.t. IEQ_ending_positions {i in {1..Nv}, j in {1..4}, line_index in {1..4}}:
TerminalLine[i,line_index,1] * xc[i,j,Nfe] + TerminalLine[i,line_index,2] * yc[i,j,Nfe] + TerminalLine[i,line_index,3] <= 0;
...
########## Collison avoidance ##########
s.t. VehicleToVehicle {i1 in {1..(Nv-1)}, i2 in {(i1+1)..Nv}, j1 in {1..4}, j2 in {1..4}, k in {0..IX}}:
(xc[i1,j1,k]-xc[i2,j2,k])^2 + (yc[i1,j1,k]-yc[i2,j2,k])^2 >= 8;

s.t. VehicleToObstacle {i in {1..Nv}, j in {1..Nobs}, k in {1..4}, m in {0..IX}}:
(xc[i,k,m]-OC[j,m,1])^2 + (yc[i,k,m]-OC[j,m,2])^2 >= 8;
